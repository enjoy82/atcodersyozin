...
int main(){
    ...
    n += 1;
    m += 1;
    vector<ll> alis(n), clis(n+m-1), blis(m, 1e15);
    REP(i,0,n){cin >> alis[i];}
    REP(i,0,n+m-1){cin >> clis[i];}
    REP(i,0,n+m-1){
        ll sum = 0;
        int ind = -1;
        int count = 0;
        int sumcount = 0;
        for(int l = min(i, n-1); l >= 0; l--){
            int bind = i - l;
            if(bind < 0 || bind >= m)
                continue;
            ll key = blis[bind];
            sumcount += 1;
            if(key != 1e15 || alis[l] == 0){
                sum += key * alis[l];
            }else{
                count += 1;
                ind = bind;
        ...
        if(count == 1){
            if(sumcount == 1 && alis[i-ind] == 0){
                continue;
            }else{
                blis[ind] = (clis[i] - sum) / alis[i-ind];
            }
        }
    }
    REP(i,0,m){
