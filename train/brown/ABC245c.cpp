...
int main(){
    ...
    vector<Pii> flags(n, Pii(0,0));
    flags[0].first = 1;
    flags[0].second = 1;
    REP(i,1,n){
        if(flags[i-1].first == 1 && abs(alis[i] - alis[i-1]) <= k){
            flags[i].first = 1;
        }
        if(flags[i-1].first == 1 && abs(blis[i] - alis[i-1]) <= k){
            flags[i].second = 1;
        }
        if(flags[i-1].second == 1 && abs(alis[i] - blis[i-1]) <= k){
            flags[i].first = 1;
        }
        if(flags[i-1].second == 1 && abs(blis[i] - blis[i-1]) <= k){
            flags[i].second = 1;
        }
    }
    if(flags[n-1].first || flags[n-1].second){
        cout << "Yes" << endl;
        return 0;
    }
    cout  << "No" << endl;
