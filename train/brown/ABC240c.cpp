...
int main(){
    ...
    vector<vector<bool> > dp(n+1, vector<bool>(x+100, false));
    dp[0][0] = true;
    REP(i,0,n){
        REP(l,0,x+1){
            if(dp[i][l]){
                if(l+query[i].first < x + 100)
                    dp[i+1][l+query[i].first] = true;
                if(l+query[i].second < x + 100)
                    dp[i+1][l+query[i].second] = true;
            }
        }
    }
    if(dp[n][x]){
