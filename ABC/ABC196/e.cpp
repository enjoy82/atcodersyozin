...
int main(){
    int n; cin >> n;
    vector<ll> alis(n), tlis(n);
    REP(i,0,n){
        cin >> alis[i] >> tlis[i];
    }
    int q; cin >> q;
    ll xlis[q]; //生配列
    REP(i,0,q){cin >> xlis[i];}
    SegmentTree seg(q, xlis);
    REP(i,0,n){
        if(tlis[i] == 1){
            seg.add_val(0, q, alis[i]);
        }else if(tlis[i] == 2){
            seg.update_max(0, q, alis[i]);
        }else{
            seg.update_min(0, q, alis[i]);
        }
    }
    REP(i,0,q){
